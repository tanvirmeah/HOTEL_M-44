...
const AllBookings = () => {
  ...
  const handleCheckInClick = (reservationId) => {
    ...
    if (booking) {
      ...
      setCheckInDetails({
        totalReceived: {
          method: booking.total_received?.method || 'CASH',
          amount: booking.total_received?.amount || 0,
        },
        managerAcknowledgement: false,
      });
      setTotalDue(booking.total_due);
  ...
  const handleCheckInConfirm = async () => {
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          ...
          total_received: { ...checkInDetails.totalReceived, amount: parseFloat(checkInDetails.totalReceived.amount) || 0 },
          total_due: totalDue
        })
    ...
    setCheckInDetails({
      totalReceived: { method: 'CASH', amount: 0 },
      managerAcknowledgement: false
    });
  };
  ...
  const handleTotalReceivedChange = (e) => {
    ...
    setCheckInDetails({
      ...checkInDetails,
      totalReceived: {
        ...checkInDetails.totalReceived,
        amount: receivedAmount
      }
    });

    // Calculate total due in real-time
    const booking = bookings.find(b => b.reservation_id === currentBookingId);
    if (booking) {
      const advancePayment = parseFloat(booking.advance_payment?.amount) || 0;
      const totalAmount = parseFloat(booking.total_amount) || 0;
      const calculatedTotalDue = totalAmount - advancePayment - receivedAmount;
  ...
  }).filter(booking => booking.status !== 'cancelled' && booking.status !== 'checked_out' && !booking.check_in_status);
            ...
            {bookings.find(b => b.reservation_id === currentBookingId) && (
              <>
                ...
                <div className="payment-section total-received">
                  ...
                  <select
                    name="method"
                    value={checkInDetails.totalReceived.method}
                  ...
                  <input
                    type="number"
                    name="amount"
                    value={checkInDetails.totalReceived.amount}
